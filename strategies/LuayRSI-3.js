...
var strat = {
	...
	long: function()
	{
		if(( this.trend.direction !== 'up'  ) && (inTrade==false)) // new trend? (only act on new trends)
		{
			...
			console.log('Candle close : ',this.candle.close)
	...
	short: function()
	{
		...
		if( this.trend.direction !== 'down' )
		{
			...
			// && 
			// ( 
				// this.candle.close>lastLongPrice*(1 + this.settings.thresholds.minProf)
			//  || 	
			//  this.candle.close < lastLongPrice* (1 - this.settings.thresholds.maxLoss)
			//  )
			// )
			{
				this.resetTrend();
